...
Promise.all([NRDB.data.promise, NRDB.settings.promise]).then(function () {
    ...
    factions.forEach(function (faction) {
        var label = $(
            '<label class="btn btn-default btn-sm" data-code="' +
                ...
                faction.code +
                '"><use xlink:href="/images/icons.svg#faction-' +
                faction.code +
                '"></use></svg></label>',
        );
    ...
    types.forEach(function (type) {
        var label = $(
            '<label class="btn btn-default btn-sm" data-code="' +
                ...
                '"><svg class="typeIcon" aria-label="' +
                type.code +
                '"><use xlink:href="/images/icons.svg#type-' +
                type.code +
                '"></use></svg></label>',
        );
        label.tooltip({ container: "body" });
    ...
    function findMatches(q, cb) {
        ...
        matchingCards.sort((card1, card2) => {
            ...
            if (
                card1title.startsWith(normalizedQuery) &&
                !card2title.startsWith(normalizedQuery)
            ) {
                return -1;
            }
            if (
...
function create_collection_tab(initialPackSelection) {
    ...
    NRDB.data.cycles.find({ rotated: true }).forEach(function (cycle) {
        rotated_cycles[cycle.code] = 1;
    });

    var rotated_packs = Array();
    NRDB.data.packs.find().forEach(function (pack) {
        if (rotated_cycles[pack.cycle.code]) {
            rotated_packs[pack.code] = 1;
        }
    });
        ...
        $("#pack_code")
            ...
            .each(function () {
                $(this).prop(
                    "checked",
                    Boolean(
                        STARTUP_CYCLES.has($(this).prop("name")) ||
                            STARTUP_PACKS.has($(this).prop("name")),
                    ),
                );
            });
        ...
        $("#pack_code")
            ...
            .each(function () {
                $(this).prop(
                    "checked",
                    Boolean(
                        NSG_ONLY_CYCLES.has($(this).prop("name")) ||
                            NSG_ONLY_PACKS.has($(this).prop("name")),
                    ),
                );
            });
        update_collection_packs();
        ...
        $("#pack_code")
            ...
            .each(function () {
                $(this).prop(
                    "checked",
                    !Boolean(
                        rotated_cycles[$(this).prop("name")] ||
                            rotated_packs[$(this).prop("name")],
                    ),
                );
            });
        update_collection_packs();
    ...
    _.sortBy(NRDB.data.cycles.find(), "position")
        .reverse()
        .forEach(function (cycle) {
            var packs = _.sortBy(
                NRDB.data.packs.find({ cycle_code: cycle.code }),
                "position",
            ).reverse();
            if (cycle.size === 1) {
                if (packs.length) {
                ...
                var $toggle = $(
                    '<div class="checkbox" data-toggle="checklist"><label><input type="checkbox" name="' +
                        cycle.code +
                        '">' +
                        cycle.name +
                        "</label></div>",
...
$(function () {
    ...
    $(".search-buttons").on("click", "label", function (event) {
        ...
        if (dropdown) {
            ...
            if (
                (!event.shiftKey &&
                    NRDB.settings.getItem("buttons-behavior") ===
                        "exclusive") ||
                (event.shiftKey &&
                    NRDB.settings.getItem("buttons-behavior") === "cumulative")
            ) {
                if (!event.altKey) {
    ...
    $("#filter-text").on({
        input: function (event) {
            var q = $(this).val();
            if (q.match(/^\w[:<>!]/))
                NRDB.smart_filter.handler(q, refresh_collection);
...
function build_div(record) {
    ...
    switch (Number(NRDB.settings.getItem("display-columns"))) {
        case 1:
            var imgsrc =
                record.faction_code.substr(0, 7) === "neutral"
                    ? ""
                    : '<svg class="typeIcon" aria-label="' +
                      record.faction_code +
                      '"><use xlink:href="/images/icons.svg#faction-' +
            ...
            div = $(
                '<tr class="card-container" data-index="' +
                    ...
                    record.title +
                    "</a> " +
                    get_influence_penalty_icons(record) +
                    '</td><td class="influence influence-' +
                    record.faction_code +
                    '">' +
                    influ +
                    '</td><td class="type" title="' +
                    record.type.name +
                    '"><svg class="typeIcon" aria-label="' +
                    record.type.code +
                    '"><use xlink:href="/images/icons.svg#type-' +
...
function update_filtered(forceBuild) {
    ...
    var counter = 0,
        ...
        display_columns = NRDB.settings.getItem("display-columns");
    ...
    var matchingCards = NRDB.data.cards.find(SmartFilterQuery, {
        $orderBy: orderBy,
    });
